...
EXPECTED_COLS = {
    ...
    "STR. PD. PROG.",
    "STR. PROG.",
    "FEST. INFRASETT. A PAGAMENTO",
    "FEST. INFRASETT. A RECUPERO",
    "FERIE",
    "FERIE RX",
    "FERIE GODUTE TOTALE",
    "FERIE GODUTE RX",
    ...
    "LEGGE 104",
    "RECUPERO",
    "MISSIONE SOLO SERVIZIO",
    "CDR_DESC",
    "DESC. DIP.",
}


def detect_best_sheet_header(xls: pd.ExcelFile, max_scan_rows=70):
    ...
    for sheet in xls.sheet_names:
        ...
        for r in range(min(len(preview), max_scan_rows)):
            ...
            score = hits * 10 + len(row_vals)  # bonus per righe "larghe"
...
with st.sidebar:
    ...
    core_only = st.toggle("Solo qualifiche core (Infermiere/OSS/TSRM/TSLB/Fisioterap/Autisti)", value=True)
...
if override:
    with st.sidebar:
        sheet_override = st.selectbox("Foglio", sheet_names, index=sheet_names.index(meta["sheet"]) if meta["sheet"] in sheet_names else 0)
...
with tab1:
    ...
    try:
        analisi, df_scope = build_analisi(df_f, only_in_force=only_in_force, core_only=core_only)
        ...
        if len(df_scope) == 0:
            st.warning("Scope vuoto: prova a disattivare 'Solo qualifiche core' o usa override header.")
    except Exception as e:
...
with tab2:
    ...
    try:
        ...
        if len(df_scope) == 0:
            st.warning("Nessun dato nello scope con filtri/opzioni attuali.")
            c_qual_dbg = find_col(df_f, ["QUALIFICA.1", "QUALIFICA"], contains=True)
            if c_qual_dbg:
